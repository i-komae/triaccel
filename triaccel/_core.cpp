// triaccel.cpp
//
// このファイルは triaccel の C++ バックエンド実装（pybind11
// バインディング）です。 役割：
...
// 事前計算済みの RA/Dec
// 配列を使ってイベント・ヒストグラムを更新（再計算を回避）
static void update_event_histograms(
    int N, const std::vector<double> &ra_deg_arr,
    const std::vector<double> &dec_deg_arr, int tid, int bins_ra,
...
static SiteC make_site(double lat_deg, double lon_deg, double zmax_deg, int n) {
  ...
  return SiteC{std::sin(phi), std::cos(phi),           std::sin(lam),
               std::cos(lam), std::sin(rad(zmax_deg)), n};
}
...
static std::vector<SiteC> parse_sites(py::sequence sites) {
  ...
  for (auto item : sites) {
    ...
    if (n <= 0)
      continue;
    v.push_back(make_site(d["lat"].cast<double>(), d["lon"].cast<double>(),
...
static void gen_site_eci(const SiteC &S, int offset, RNG &rng,
  ...
  for (int i = 0; i < S.n; ++i) {
    ...
    // ENU→ECEF
    double xe =
        e * (-S.slam) + nN * (-S.sphi * S.clam) + uU * (S.cphi * S.clam);
...
static void generate_events_and_cache(const std::vector<SiteC> &S, RNG &rng,
                                      ThreadBuf &B) {
  int off = 0;
...
static void dump_k_cliques_u128(int K, const std::vector<uint64_t> &m0,
                                const std::vector<uint64_t> &m1, int N,
                                std::ofstream &cf) {
  if (K < 2 || N <= 0) {
    return;
  }

  auto and_with_nb = [&](uint64_t c0, uint64_t c1, int v, uint64_t &o0,
                         uint64_t &o1) {
    o0 = c0 & m0[v];
    o1 = c1 & m1[v];
  };
  ...
  std::function<void(int, int, uint64_t, uint64_t)> dfs =
      [&](int last, int depth, uint64_t c0, uint64_t c1) {
        if (depth == K) {
          for (int i = 0; i < K; ++i) {
            if (i) {
              cf << '\t';
...
static void process_smallN_trial(
    int t, int M, int N, int tid, int cluster_size, double cos_thr, int bins_ra,
    const std::function<void(double, double, int &, int &)> &bin2d,
    const std::string &log_text_dir, ThreadBuf &B,
    std::vector<std::vector<long long>> &Evt2d_loc,
  ...
  if constexpr (Hist) {
    if (cluster_size == 2) {
      kcnt = count_pairs_hist_u128(N, tid, B.m0, B.m1, B.x, B.y, B.z,
                                   /*mark_members=*/Debug, B.in_cluster,
                                   bins_ra, bin2d, Tri2d_loc);
    } else {
      if constexpr (Debug) {
        kcnt = count_k_cliques_hist_u128(cluster_size, B.m0, B.m1, N,
                                         &B.in_cluster, true, tid, B.x, B.y,
                                         B.z, bins_ra, bin2d, Tri2d_loc);
      } else {
        kcnt = count_k_cliques_hist_u128_nodebug(cluster_size, B.m0, B.m1, N,
                                                 tid, B.x, B.y, B.z, bins_ra,
                                                 bin2d, Tri2d_loc);
      }
    }
  } else {
    if (cluster_size == 2) {
      kcnt = count_pairs_u128(B.m0, B.m1, N, Debug ? &B.in_cluster : nullptr,
                              Debug);
    } else {
      if constexpr (Debug) {
        kcnt = count_k_cliques_nohist_u128(cluster_size, B.m0, B.m1, N,
                                           &B.in_cluster, true);
      } else {
        kcnt = count_k_cliques_nohist_u128_nodebug(cluster_size, B.m0, B.m1, N);
      }
    }
  }
  ...
  if constexpr (Debug) {
    ...
    std::fprintf(stderr, "t=%d/%d count k-cliques (k=%d, fastpath=%s)\n", t + 1,
                 M, cluster_size, fastpath);
    try {
      char evname[64];
      std::snprintf(evname, sizeof(evname), "sim_%05d_events.txt", t + 1);
      char clqname[64];
      std::snprintf(clqname, sizeof(clqname), "sim_%05d_cliques_k%d.txt", t + 1,
                    cluster_size);
...
static void dump_k_cliques_bitadj(int K, const std::vector<uint64_t> &NB, int N,
                                  int W, std::ofstream &cf) {
  if (K < 2 || N <= 0) {
    return;
  }

  auto and_row = [&](const std::vector<uint64_t> &a, int v,
...
#if defined(__AVX2__)
  __m256d v_cos_thr = _mm256_set1_pd(cos_thr);
  for (int i2 = 0; i2 < N; ++i2) {
    double xi = B.x[i2], yi = B.y[i2], zi = B.z[i2];
    __m256d v_xi = _mm256_set1_pd(xi);
    __m256d v_yi = _mm256_set1_pd(yi);
    __m256d v_zi = _mm256_set1_pd(zi);

    int j2 = i2 + 1;
    for (; j2 <= N - 4; j2 += 4) {
      __m256d v_xj = _mm256_loadu_pd(&B.x[j2]);
      __m256d v_yj = _mm256_loadu_pd(&B.y[j2]);
      __m256d v_zj = _mm256_loadu_pd(&B.z[j2]);

      __m256d v_dot = _mm256_fmadd_pd(
          v_xi, v_xj, _mm256_fmadd_pd(v_yi, v_yj, _mm256_mul_pd(v_zi, v_zj)));

      __m256d v_cmp = _mm256_cmp_pd(v_dot, v_cos_thr, _CMP_GE_OQ);
...
#pragma unroll
      for (int k = 0; k < 4; ++k) {
        if ((mask >> k) & 1) {
          int target_j = j2 + k;
          set_edge(i2, target_j);
          set_edge(target_j, i2);
          edges_undirected++;
          degv[i2]++;
          degv[target_j]++;
        }
...
#elif defined(__aarch64__)
  float64x2_t v_cos_thr = vdupq_n_f64(cos_thr);
  for (int i2 = 0; i2 < N; ++i2) {
    double xi = B.x[i2], yi = B.y[i2], zi = B.z[i2];
    float64x2_t v_xi = vdupq_n_f64(xi);
    float64x2_t v_yi = vdupq_n_f64(yi);
    float64x2_t v_zi = vdupq_n_f64(zi);

    int j2 = i2 + 1;
    for (; j2 <= N - 2; j2 += 2) {
      float64x2_t v_xj = vld1q_f64(&B.x[j2]);
      float64x2_t v_yj = vld1q_f64(&B.y[j2]);
      float64x2_t v_zj = vld1q_f64(&B.z[j2]);

      float64x2_t v_dot = vmulq_f64(v_xi, v_xj);
      v_dot = vfmaq_f64(v_dot, v_yi, v_yj);
      v_dot = vfmaq_f64(v_dot, v_zi, v_zj);

      uint64x2_t v_cmp = vcgeq_f64(v_dot, v_cos_thr);
  ...
  if (cluster_size == 2) {
    long long pair_cnt;
    if constexpr (Hist) {
      pair_cnt = count_pairs_hist_bitadj(NB, N, W, tid, B.x, B.y, B.z,
                                         /*mark_members=*/Debug, B.in_cluster,
                                         bins_ra, bin2d, Tri2d_loc);
    } else {
      ...
      if constexpr (Debug) {
        ...
        if (check != pair_cnt) {
          std::fprintf(
              stderr,
              "[warn] pair_cnt mismatch: edges_undirected=%lld, recount=%lld\n",
              pair_cnt, check);
        }
      }
    }
    ...
    long long kcnt = count_k_cliques_bitadj(
        cluster_size, NB, N, W, Debug ? &B.in_cluster : nullptr, Debug, tid,
        &B.x, &B.y, &B.z, Hist, bins_ra, &bin2d, &Tri2d_loc);
    counts_vec[t] = (int32_t)kcnt;
  }
      ...
      if (cluster_size == 3) {
        ...
        for (int i0 = 0; i0 < N; ++i0) {
          for (int j0 = i0 + 1; j0 < N; ++j0) {
            ...
            for (int w = wb; w < W; ++w) {
              uint64_t c = NB[(size_t)i0 * (size_t)W + (size_t)w] &
                           NB[(size_t)j0 * (size_t)W + (size_t)w];
              if (w == wb) {
                uint64_t mask = 0ULL;
                if (bb < 63) {
                  mask = ~((1ULL << (bb + 1)) - 1ULL);
...
static void parse_bins_option_or_default(py::object bins, int &bins_ra,
                                         int &bins_dec) {
  bins_ra = 72;
  bins_dec = 36;
  if (bins.is_none()) {
...
static void run_simulation_and_aggregate(
    int M, const std::vector<SiteC> &S, int N, double cos_thr, int cluster_size,
    double ra_lo, double ra_hi, int bins_ra, int bins_dec,
    bool return_histograms, py::array_t<int32_t> &counts,
    std::vector<long long> &H_evt2d, std::vector<long long> &H_tri2d,
    const std::function<void(double, double, int &, int &)> &bin2d, bool debug,
    bool progress, py::object threads, uint64_t seed_val,
  ...
  // スレッド起動前に debug/hist に応じて固定の worker
  // を選択する（分岐はここだけ）
  auto make_worker = [&](auto tag_debug, auto tag_hist) {
    ...
    return [&, Debug, Hist](int tid, int t_begin, int t_end) {
      ...
      for (int t = t_begin; t < t_end; ++t) {
        ...
        if constexpr (Hist) {
          update_event_histograms(N, B.ra_deg_arr, B.dec_deg_arr, tid, bins_ra,
                                  bin2d, Evt2d_loc);
        }

        if (N <= 128) {
          process_smallN_trial<Debug, Hist>(t, M, N, tid, cluster_size, cos_thr,
                                            bins_ra, bin2d, log_text_dir, B,
                                            Evt2d_loc, Tri2d_loc, counts_vec);
        } else {
        ...
        update_progress_bar(use_progress, c, M, start_steady,
                            last_progress_time, print_mtx, step);
      }
    };
  };
  ...
  while (!cancel.load(std::memory_order_relaxed) &&
         done.load(std::memory_order_relaxed) < M) {
    {
...
simulate(int M,
         ...
         bool return_histograms = false, py::object threads = py::none(),
         bool progress = true, bool debug = false, int cluster_size = 3) {
  ...
  // 出力：トリプレット/クリーク数（counts 配列）。debug 時は M を 1000
  // にクランプ
  if (debug && M > 1000) {
  ...
  run_simulation_and_aggregate(M, S, N, cos_thr, cluster_size, ra_lo, ra_hi,
                               bins_ra, bins_dec, return_histograms, counts,
                               H_evt2d, H_tri2d, bin2d, debug, progress,
                               threads, seed_val, log_text_dir);
  ...
  if (return_histograms) {
    ...
    for (int i = 0; i <= bins_ra; ++i)
      ra_edges.mutable_at(i) =
          ra_lo + wra * (double)i; // [-180, 180]（右端は境界）
...
PYBIND11_MODULE(_core, m) {
  m.doc() = "Triplet simulation: counts only by default; optional 1D/2D RA/Dec "
            "hist with unified bins; seed optional";
  m.def("simulate", &simulate, py::arg("M"), py::arg("sites"), py::arg("d_deg"),
        py::arg("seed") = py::none(), py::arg("bins") = py::none(),
        py::arg("return_histograms") = false, py::arg("threads") = py::none(),
        py::arg("progress") = true, py::arg("debug") = false,
